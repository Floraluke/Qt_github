    ...
    ui(new Ui::QWMainWind)
{
    ...
    // 创建工具按钮
    QToolButton *toolButtonAbout = new QToolButton(this);
    toolButtonAbout->setText(tr("关于"));

    // 将按钮添加到工具栏
    ui->mainToolBar->addWidget(toolButtonAbout);

    iniUI(); // 手工初始化UI
    iniSignalSlots(); // 信号与槽关联

    // 添加About按钮的信号槽连接
    connect(toolButtonAbout, &QToolButton::clicked,
            this, &QWMainWind::on_toolButtonAbout_clicked);

    // 设置初始文本
    ui->txtEdit->setPlainText("欢迎使用文本编辑器");
}
...
void QWMainWind::on_comboFont_currentIndexChanged(const QString &arg1)
{
    ...
    QTextCharFormat fmt;
    fmt.setFontFamilies({arg1}); // 使用setFontFamilies代替已弃用的setFontFamily
    ui->txtEdit->mergeCurrentCharFormat(fmt);
}
...
void QWMainWind::on_toolButtonAbout_clicked()
{
